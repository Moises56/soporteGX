...
const K2btToggleBar = class {
  ...
  getToggleContent(values, containsDetails, containsIcons, containsTrailingText) {
    if (values.length == 0) {
      return index.h("div", { class: "K2BT_ToggleBar_NoItems" }, this.noresultsfoundtext);
    }
    else {
      ...
      return (index.h("div", { class: containerClasses }, atomic.length > 0 ? (index.h("div", { class: "K2BT_ToggleBarItemContainer" }, atomic.map(item => this.getItemContent(item, containsIcons, containsTrailingText, containsDetails)))) : (''), currentLevelCategories.map(c => {
        return (index.h("div", { class: "K2BT_ToggleBar_CategoryContainer" }, index.h("div", { class: "K2BT_ToggleBarCategoryName" }, c.description), this.getToggleContent(c.items, containsDetails, containsIcons, containsTrailingText)));
      })));
    }
  }
  getItemContent(item, containsIcons, containsTrailingText, containsDetails) {
    return (index.h("div", { class: this.itemIsSelected(item) ? 'K2BT_ToggleBarItem K2BT_ToggleBarItemSelected' : 'K2BT_ToggleBarItem', ref: c => {
        if (this.itemIsSelected(item))
          this.selectedElement = c;
      }, onClick: () => this.setValue(item.value.trim()) }, this.getIconHTMLIfNecessary(containsIcons, item), index.h("div", { class: "K2BT_ToggleBarItem_TextSection" }, index.h("div", { class: "K2BT_ToggleBarItem_TextContainer" }, index.h("span", { class: "K2BT_ToggleBarDescription" }, item.description), containsTrailingText ? index.h("span", { class: "K2BT_ToggleBarTrailingText" }, item.trailingText) : ''), index.h("div", { class: "K2BT_ToggleBarItem_TextContainer" }, containsDetails ? index.h("span", { class: "K2BT_ToggleBarSubtitle" }, item.detail) : ''))));
  }
  getIconHTMLIfNecessary(containsIcons, item) {
    return containsIcons ? (index.h("div", { class: "K2BT_ToggleBarIconContainer" }, index.h("img", { class: "K2BT_ToggleBarIcon", src: item.imageUrl, onError: e => this.onImageError(e) }))) : ('');
  }
  GetContainerClasses() {
  ...
  getSelectedItemsContent() {
    if (this.value == null)
      ...
      var vals = utils.ControlInfoValue.getAtomicValues_impl(this.values).filter(value => this.itemIsSelected(value));
      if (vals.length > 0)
  ...
  render() {
    var containsDetails = utils.ControlInfoValue.containsDetails(utils.ControlInfoValue.getAtomicValues_impl(this.values));
    var containsIcons = utils.ControlInfoValue.containsIcons(utils.ControlInfoValue.getAtomicValues_impl(this.values));
    var containsTrailingText = utils.ControlInfoValue.containsTrailingText(utils.ControlInfoValue.getAtomicValues_impl(this.values));
    var valueList = this.getValues();
    if (this.enabled) {
      return this.getToggleContent(valueList, containsDetails, containsIcons, containsTrailingText);
    }
    else {
      return (index.h("p", { class: "form-control-static" }, index.h("span", { class: this.readonlyclass, "data-gx-readonly": "" }, this.getSelectedItemsContent())));
    }
  }
  getValues() {
  ...
  componentDidLoad() {
    var currentValues = utils.ControlInfoValue.getAtomicValues_impl(this.values);
...
exports.k2bt_toggle_bar = K2btToggleBar;
