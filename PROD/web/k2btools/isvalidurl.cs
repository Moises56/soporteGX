...
namespace GeneXus.Programs.k2btools {
   public class isvalidurl : GXProcedure
   {
      ...
      void executePrivate( )
      {
         ...
         AV9IsValidURL = GxRegex.IsMatch(AV8URL,"^((?:[a-zA-Z]+:(//)?)?((?:(?:[a-zA-Z]([a-zA-Z0-9$\\-_@&+!*"+"\""+"'(),]|%[0-9a-fA-F]{2})*)(?:\\.(?:([a-zA-Z0-9$\\-_@&+!*"+"\""+"'(),]|%[0-9a-fA-F]{2})*))*)|(?:(\\d{1,3}\\.){3}\\d{1,3}))(?::\\d+)?(?:/([a-zA-Z0-9$\\-_@.&+!*"+"\""+"'(),=;: ]|%[0-9a-fA-F]{2})+)*/?(?:[#?](?:[a-zA-Z0-9$\\-_@.&+!*"+"\""+"'(),=;: /]|%[0-9a-fA-F]{2})*)?)?\\s*$");
         this.cleanup();
      }
