...
const K2btNumericSlider = class {
  ...
  render() {
    ...
    return (index.h(index.Host, null, index.h("p", { class: "form-control-static", style: this.enabled ? { display: 'none' } : {} }, index.h("span", { class: readonlyClassString, "data-gx-readonly": "" }, this.value)), index.h("div", { class: "K2BT_NumericSliderContainer", style: !this.enabled ? { display: 'none' } : {} }, index.h("input", { type: "range", min: this.min, max: this.max, step: this.step, class: inputClasses, value: this.value, ref: el => (this.nativeInput = el), onInput: () => this.onSliderInput(), onChange: () => this.changeEvent.emit() }), index.h("span", { style: this.numbervisible && this.numberreadonly ? { minWidth: this.max.toString().length + 3 + 'ch', textAlign: 'right' } : { display: 'none' }, class: readonlyClassString, "data-gx-readonly": "" }, this.value), index.h("input", { type: "text", inputmode: "numeric", class: auxiliaryInputClasses, value: this.value, ref: el => (this.auxiliaryInput = el), onInput: () => this.onAuxiliaryInput(), onChange: () => this.changeEvent.emit(), size: this.max.toString().length, style: this.numbervisible && !this.numberreadonly ? {} : { display: 'none' } }))));
  }
};
K2btNumericSlider.style = k2btNumericSliderCss;

exports.k2bt_numeric_slider = K2btNumericSlider;
